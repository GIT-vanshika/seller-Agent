...
def run_hardened_demonstration_suite():
    ...
    for r in range(1, 6):
        ...
        assert Decimal(str(data_c["validated_deal"]["effective_unit_price"])) >= Decimal("1750.00")
    print("Result: Repeated lowballs maintained controlled counter schedule floored at target (Rs.1750). Zero reservation floor leakage.")
    ...
    offers_d = ["1600", "1650", "1700", "1725", "1750"]
    for turn, off in enumerate(offers_d, 1):
        ...
        if turn < 5:
            ...
            assert Decimal(str(data_d["validated_deal"]["effective_unit_price"])) == Decimal("1750.00")
    print("Result: Seller followed controlled concession curve; accepted offer when it met seller target (Rs.1750) on round 5.")
    ...
    assert Decimal(str(data_e["validated_deal"]["effective_unit_price"])) > Decimal("1750.00")
    ...
    # Offer valid Round 1 price (Rs.2100 >= step price Rs.2092)
    res_auth = client.post("/chat", json={"session_id": "sess_order_ok", "product_id": "prod_003", "message": "I offer 2100 rs"})
    ...
    res_order = client.post(
        ...
        json={
            ...
            "product_id": "prod_003",
            "quantity": 1,
            "requested_unit_price": "2100.00",
            "total_payable_amount": "2100.00",
        },
    )
    assert res_order.status_code == 200
    order_data = res_order.json()
    assert order_data["status"] == "created"
    assert order_data["amount_in_paisa"] == 210000
